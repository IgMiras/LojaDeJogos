...
async function cadastrarPagamento(req, res) {
	...
	try {
		...
		const pagamentoSalvo = await pagamentoModel.save();
		res.status(200).json(pagamentoSalvo._id);
	} catch (err) {
...
async function listarTodosPagamentos(req, res) {
	try {
		const pagamentos = await PagamentoModel.find();
...
module.exports = { cadastrarPagamento, listarTodosPagamentos };
